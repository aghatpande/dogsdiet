...
```
All categorical and numerical variables are currently 'character variables. Need to convert them to cotegorical or numerical as appropriate

```{r, classifying variables correctly}
samplesCategorizedAll <- samplesCategorizedAll %>% mutate(category=as.factor(category),Remission=as.factor(Remission),CCECAI=as.numeric(CCECAI),dogID=as.factor(dogID))
```
