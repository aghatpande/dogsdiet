...
The 'diseased' category can be further split into samples at various days into treatment, samples from diet responsive and unresponsive dogs, samples from the same dog at various time points into treatment. 
...
```{r, creating sub-categories}
waterControl <- CategorizingSamples %>%  filter (grepl ("Water", SampleName)) %>% mutate (category = 'water_control')
dnaXtractControl <- CategorizingSamples %>%  filter (grepl ("DNA", SampleName)) %>% mutate (category = 'DNA_extraction_control')
Mock <- CategorizingSamples %>%  filter (grepl ("Mock", SampleName)) %>% mutate (category = 'mockCommunity')
Healthy <- CategorizingSamples %>%  filter (grepl ("Healthy", SampleName)) %>% mutate (category = 'healthy')
Diseased <- CategorizingSamples %>% filter(grepl("Day", SampleName)) %>% mutate(category = 'diseased')
day0<- CategorizingSamples %>% filter(grepl("Day0", SampleName)) %>% mutate(category = 'day0CE')
day14<- CategorizingSamples %>% filter(grepl("Day14", SampleName)) %>% mutate(category = 'day14CE')
day28<- CategorizingSamples %>% filter(grepl("Day28", SampleName)) %>% mutate(category = 'day28CE')
day35<- CategorizingSamples %>% filter(grepl("Day35", SampleName)) %>% mutate(category = 'day35CE')
day42<- CategorizingSamples %>% filter(grepl("Day42", SampleName)) %>% mutate(category = 'day42CE')
day56<- CategorizingSamples %>% filter(grepl("Day56", SampleName)) %>% mutate(category = 'day56CE')
dogIDs <- CategorizingSamples %>% filter(grepl)
samplesCategorizedSub1 <- bind_rows(waterControl,dnaXtractControl,Healthy,Mock,day0,day14,day28,day35,day42,day56)
```
  
To generate a file called "ENTiCEampliconMetadata.tsv" or "ENTiCEampliconsub1Metadata.tsv", we will use the previously created tibbles: samplesCategorized or samplesCategorizedsub1 respectively
