...
Imported waterCntrlmanifest as PairedEndFastqManifestPhred33V2 to waterCntrldemux.qza
Imported DNAxtractCntrlmanifest as PairedEndFastqManifestPhred33V2 to DNAxtractCntrldemux.qza
Imported MockCntrlmanifest as PairedEndFastqManifestPhred33V2 to mockCntrldemux.qza
...
Qiime 2 reported: Saved Visualization to: waterCntrldemux.qzv; Saved Visualization to: DNAxtractCntrldemux.qzv; Saved Visualization to: mockCntrldemux.qzv
  ...
  *The UPenn team used an average Phred score of 20 for each sample and removed sequences >256 nt and < 250 nt in length per the paper*
  ...
  There were 14 samples labelled as water controls. The reads obtained in each sample are summarized below
  
```{r, water controls reads summary}
watercntrlreads <- read_csv("waterCntrldemuxdreads.csv")
waterCntrlsummary <- watercntrlreads %>% ggplot() + geom_histogram(mapping = aes(x = `Sequence count`, fill = `Sample name`)) + labs(x="reads",y="frequency")
waterCntrlsummary                                                
```
...
Saved FeatureTable[Frequency] to: watercntrl-table.qza
Saved FeatureData[Sequence] to: watercntrl-rep-seqs.qza
Saved SampleData[DADA2Stats] to: watercntrl-stats.qza

Saved FeatureTable[Frequency] to: dnaxtractcntrl-table.qza
Saved FeatureData[Sequence] to: dnaxtractcntrl-rep-seqs.qza
Saved SampleData[DADA2Stats] to: dnaxtractcntrl-stats.qza

Saved FeatureTable[Frequency] to: mockcntrl-table.qza
Saved FeatureData[Sequence] to: mockcntrl-rep-seqs.qza
Saved SampleData[DADA2Stats] to: mockcntrl-stats.qza
